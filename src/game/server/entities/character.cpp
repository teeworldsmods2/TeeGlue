...
void CCharacter::FireWeapon()
{
	...
	case WEAPON_HAMMER:
	{
		...
		IBuilding *Building = (IBuilding *)GameWorld()->ClosestEntity(ProjStartPos, 64.f, CGameWorld::ENTTYPE_BUILDING, 0);
		if (Building)
			Building->TakeDamage(GetPlayer()->GetCID(), WEAPON_HAMMER);
...
void CCharacter::TickDefered()
{
	...
	for (int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		{
			if (Events & COREEVENTFLAG_GROUND_JUMP && i != m_pPlayer->GetCID())
				GameServer()->CreateSound(m_Pos, SOUND_PLAYER_JUMP, CmaskOne(i));

			if (Events & COREEVENTFLAG_HOOK_ATTACH_PLAYER)
				GameServer()->CreateSound(m_Pos, SOUND_HOOK_ATTACH_PLAYER, CmaskOne(i));
			if (Events & COREEVENTFLAG_HOOK_ATTACH_GROUND && i != m_pPlayer->GetCID())
				GameServer()->CreateSound(m_Pos, SOUND_HOOK_ATTACH_GROUND, CmaskOne(i));
			if (Events & COREEVENTFLAG_HOOK_HIT_NOHOOK && i != m_pPlayer->GetCID())
...
void CCharacter::SetTestBuilding()
{
	new IBuilding(GameWorld(), GetPos(), vec2(GetPos().x + (32.f * 5.f), GetPos().y - (32.f * 5.f)), 200, 5, GetPlayer()->GetTeam());
